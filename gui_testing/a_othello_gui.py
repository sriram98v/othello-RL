...
import a_test_othello
import a_othello_gui_models
import tkinter
...
DEFAULT_ROWS = 8
DEFAULT_COLUMNS = 8
DEFAULT_FIRST_PLAYER = a_test_othello.BLACK
DEFAULT_TOP_LEFT_PLAYER = a_test_othello.WHITE
DEFAULT_VICTORY_TYPE = a_test_othello.MOST_CELLS

# GUI Constants
BACKGROUND_COLOR = a_othello_gui_models.BACKGROUND_COLOR
GAME_HEIGHT = 300
GAME_WIDTH = 300
...
class OthelloGUI:
    def __init__(self):
        ...
        # TODO: check edit the game state here
        self._game_state = a_test_othello.OthelloGame(self._rows, self._columns,
        ...
        self._board = a_othello_gui_models.GameBoard(self._game_state, GAME_WIDTH, GAME_HEIGHT, self._root_window)
        self._black_score = a_othello_gui_models.Score(a_test_othello.BLACK, self._game_state, self._root_window)
        self._white_score = a_othello_gui_models.Score(a_test_othello.WHITE, self._game_state, self._root_window)
        self._player_turn = a_othello_gui_models.Turn(self._game_state, self._root_window)
    ...
    def _configure_game_settings(self) -> None:
        ...
        dialog = a_othello_gui_models.OptionDialog(self._rows, self._columns,
                                             self._first_player, self._top_left_player,
                                             self._victory_type)
        dialog.show()
